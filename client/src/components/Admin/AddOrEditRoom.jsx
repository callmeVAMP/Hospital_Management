...
const BookorEditRoomForm = ({ open, onClose, onSave, roomData }) => {
  ...
  const handleSubmit = () => {
    //if (!formData.roomNumber || !formData.roomType || !formData.floor|| !formData.bedCapacity || !formData.roomRate || !formData.roomCategory) {
      if (!formData.roomNumber || !formData.roomType || !formData.bedCapacity || !formData.roomRate || !formData.roomCategory) {
      alert("Please fill required fields: Room's Number, Room's Type, Bed's Capacity, Rate, Category");
      return;
    }
    if (onSave) {
      onSave(formData);
