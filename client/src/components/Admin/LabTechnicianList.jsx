...
export default function LabTechniciansTable() {
  ...
  const [anchorEl, setAnchorEl] = useState(null);
  const [openForm, setOpenForm] = useState(false);
  const [editingTech, setEditingLabTech] = useState(null);
  const [labtechToDelete, setLabTechToDelete] = useState(null);
  ...
  const updateLabTech = async (data) => {
    try {
      ...
      const response = await axios.put(
        `http://localhost:8000/admin/update_lab_technician/${data.id}`, backendData);
  ...
  // Fixed delete function
  const deleteLabTech = async (id) => {
    if (!deleteTarget) return;
    
    try {
      const response = await axios.delete(`http://localhost:8000/admin/delete_lab_technician/${deleteTarget.id}`);
      
      if (response.data.success) {
        setLabTechs(prev => prev.filter(lt => lt.id !== id));
        setSnackBarInfo({ message: "Deleted successfully!", severity: "success" });
      } else {
  ...
  return (
    <Box sx={{ height: 600, width: "100%", p: 2 }}>
      <Box sx={{
        ...
        <Typography variant="h6">Lab Technicians</Typography>
        <Box display="flex" alignItems="center" gap={2}>
          <Box sx={{ backgroundColor: "white", px: 2, py: 0.5, borderRadius: 1, display: "flex", gap: 1, alignItems: "center" }}>
            <SearchIcon fontSize="small" />
            <TextField
              variant="standard" placeholder="Search"
              InputProps={{ disableUnderline: true }}
      ...
      <DeleteDialog
        ...
        data={labtechToDelete}
        fields={fieldsToDisplay}
        onCancel={() => {
          setDeleteDialogOpen(false);
          setLabTechToDelete(null);
        }}
        onConfirm={deleteLabTech}
      />
