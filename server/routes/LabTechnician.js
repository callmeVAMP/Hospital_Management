...
//profile of lab tech
router.get('/LAB_tech_profile', async (req, res) => {

try {
    const [results, fields] = await connection.query(
    'SELECT L.HID,L.LabID,H.Hname,H.HAddr,H.HPhNo,H.HGender from LabTechnician L join HealthcareProf H on L.HID = H.HID '
    );
...
//list of test to be conducted
router.get('/List_of_test', async (req, res) => {

    try {
        const [results, fields] = await connection.query(
        `SELECT p.PID,p.PName,a.DTime, t.TestName from Appointment a 
